...
{
    public class GetTaskItemByIdQueryHandler : IRequestHandler<GetTaskItemByIdQuery, TaskItemByIdVm>
    {
        private readonly ITaskManagerDbContext _taskManagerDbContext;
        private readonly ICurrentUserService _currentUserService;
        private readonly IAccessControlService _accessControlService;

        public GetTaskItemByIdQueryHandler(ITaskManagerDbContext taskManagerDbContext, ICurrentUserService currentUserService, IAccessControlService accessControlService)
        {
        ...
        public async Task<TaskItemByIdVm> Handle(GetTaskItemByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            await _accessControlService.CheckAccess(userId, request.TaskItemId, PermissionLevel.ReadOnly, false, cancellationToken);
            ...
            return new TaskItemByIdVm { TaskLists = taskListsDto };
        }
        ...
        private async Task<string> GetTaskListCreatorId(GetTaskItemByIdQuery request, CancellationToken cancellationToken)
        {
        ...
        private static List<GetTaskListByIdDto> MapTaskListsToDto(List<TaskList> taskLists, int taskItemId, string creatorName)
        {
            return taskLists
                .Where(tl => tl.TaskItems.Any())
                .Select(tk => new GetTaskListByIdDto
                {
                    ...
                    TaskItems = tk.TaskItems
                        ...
                        .Select(ti => new GetTaskItemByIdDto
                        {
